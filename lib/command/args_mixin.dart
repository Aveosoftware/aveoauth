...
mixin ArgsMixin {
  final List<String> _args = AuthCli.arguments;
...
List<String> _getArgs() {
  var args = List.of(AuthCli.arguments);
  _removeDefaultArgs(args);
...
int _getIndexArg(String arg) {
  return AuthCli.arguments.indexWhere((element) => element == arg);
}

String _getArg(String arg) {
  var index = _getIndexArg(arg);
  if (index != -1) {
    if (index + 1 < AuthCli.arguments.length) {
      index++;
      return AuthCli.arguments[index];
    } else {
